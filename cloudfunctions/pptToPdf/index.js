...
/**
 * PPT转PDF云函数 - 使用LibreOffice进行转换
 * 接收PPT文件ID，转换为PDF并返回PDF文件ID
 *
...
exports.main = async (event, context) => {
    ...
    try {
        console.log('开始处理PPT文件:', fileName);
        ...
        const pptBuffer = downloadResult.fileContent;

        // 2. 保存PPT文件到临时目录
        const pptPath = path.join(tmpDir, `input_${Date.now()}.${fileName.split('.').pop()}`);
        fs.writeFileSync(pptPath, pptBuffer);
        console.log('PPT文件已保存到:', pptPath);

        // 3. 使用LibreOffice转换PPT为PDF
        console.log('开始转换PPT为PDF...');
        ...
        try {
            ...
            // --outdir: 输出目录
            const command = `libreoffice --headless --convert-to pdf --outdir ${tmpDir} ${pptPath}`;
            ...
            // 5. 上传PDF到云存储
            const outputFileName = fileName.replace(/\.(ppt|pptx)$/i, '.pdf');
            ...
            if (conversionError.message.includes('libreoffice')) {
                ...
                /*
                const convertioResult = await convertUsingConvertio(pptBuffer, fileName);
                return convertioResult;
                */

                throw new Error('云函数环境暂不支持PPT转换，请联系管理员配置LibreOffice或第三方转换服务');
            }
        ...
        try {
            ...
            files.forEach(file => {
                if (file.startsWith('input_') &&
                    (file.endsWith('.ppt') || file.endsWith('.pptx') || file.endsWith('.pdf'))) {
        ...
        return {
            ...
            details: error.stack,
            tip: '如需使用PPT转PDF功能，请确保云函数环境已安装LibreOffice或配置第三方转换服务'
        };
    }
};
