// pages/markdown-to-pdf/markdown-to-pdf.js
Page({
    /**
     * 页面的初始数据
     */
    ...
    onLoad(options) {
        ...
        if (!wx.cloud) {
            wx.showModal({
                title: '云开发未开通',
                content: '请先开通云开发功能才能使用Markdown转PDF',
                showCancel: false
            });
        }
    },
    ...
    async convertToPDF() {
        ...
        try {
            // 上传文件到云存储
            const cloudPath = `markdown-files/${Date.now()}_${this.data.selectedFile.name}`;
